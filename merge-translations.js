...
if (args.length >= 3) {
  const outputFile = args[3] || './merged_translations_vi_updated.xml';
  mergeTranslations(args[0], args[1], args[2], outputFile);
} else {
  console.log('Cách dùng:');
  console.log('  node merge-translations.js <EN-mới> <VI-cũ> <VI-mới> [output]');
  console.log('\nVí dụ:');
  console.log('  node merge-translations.js merged_translations.xml merged_translations_vi.xml new_content_translated_vi.xml');
  process.exit(1);
}
